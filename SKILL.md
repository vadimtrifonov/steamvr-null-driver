...
`check` is optional.
`start` performs the same authoritative preflight.
...
The duration includes startup.
The permitted range is 1 through 120 minutes.

Continue when `ok=true` and `run.phase=ready`.
Save `runId` and `run.environment`.
...
The caller owns each child process.
Stop child processes before you run `stop`.
...
Cleanup is complete when `ok=true`.
Keep the run ID until cleanup is complete.
...
Each run has unique configuration and log directories.
`run.environment` contains `VR_OVERRIDE`, `VR_CONFIG_PATH`, and `VR_LOG_PATH`.
